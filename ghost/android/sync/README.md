...
- **`runUserIssuerCall`** (onboarding și butoanele opționale din modul standard): blocul rulează exact o dată, pe propriul fir, cu o sesiune `USER_ISSUER_CALL`, pe transportul sesiunii care rulează sau, fără sesiune, pe un transport făcut READY pentru el (închis după). Așteaptă sfârșitul unei rulări liniștite; după `onWipe` sesiunea e deja închisă.
- **Ecranul de plată** (`onPaymentScreenShown`/`onPaymentScreenHidden`; ascunderea aplicației îl ascunde): închide imediat sesiunea cu relay-uri și nu pornește alta cât e afișat și încă U[20, 60] min după. Rulările liniștite și apelurile utilizatorului nu sunt sesiuni cu relay-uri.
- Nimic din ce face participantul nu atinge breakerele, bugetele, pauzele sau erorile de transport ale sesiunii (T19; `SessionParticipantTest`).
