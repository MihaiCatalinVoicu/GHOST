 ...
 * flags and the moment the payment screen was last visible. Plain INSERT after a read (§19.22 point 4).
 */
internal class StateStore {
    ...
    fun read(tx: SyncTransaction): EntState? = tx.sql.single(
        "SELECT schedule_seq, schedule_digest, next_invite_index, payout_salt, auto_renew_credits, alarm_flags, payment_shown_minute " +
            "FROM ent_state WHERE id = 1",
    ) { EntState(it.long(0), it.blob(1), it.long(2).toInt(), it.blob(3), it.long(4) == 1L, it.long(5).toInt(), it.longOrNull(6)) }
