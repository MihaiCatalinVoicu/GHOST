 ...
 * `start(p + 1) + 1 h`, INVITE epochs before `e_now − 1`, CREDIT epochs before `c_now − 4`); used
...
internal class Gc(private val c: EngineContext) {
    ...
    fun run(now: Long) {
        ...
        c.tx { tx ->
            ...
            c.tokens.collect(tx, Grid.week(now - Grid.HOUR) - 1, Grid.inviteEpoch(week) - 1, Grid.creditEpoch(week) - CREDIT_EPOCHS_KEPT)
