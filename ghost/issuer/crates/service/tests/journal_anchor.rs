...
//! holds no entry, while the journal holds an entry of an earlier week, decides a data-free entry
//! into it, so an idle issuer's journal gets a fresh segment every week and the segment of its last
//! transition (claim hashes, minors, digests, nullifiers, payout addresses) is pruned within the
//! 7–14-day retention: residue E31 closes while the scanner runs.
//!
//! Covered here: when an anchor is due (an idle week, also with the wallet down) and when not (an
//! empty journal, later ticks of the week, a transition before the week's first tick); a crash at
//! each fault site of the anchor (before its transaction, the entry lost, torn or durable before
//! the commit, the commit failed or done), with the exact state left before the restart and the
//! restart's replay; the prune of an idle issuer after a verified snapshot down to segments that
//! hold only anchors, on which the issuer restarts and from which a restore of that snapshot
//! replays. The same crashes, each also followed by a second one, are enumerated by I-N in
//! `tests/crash.rs` (release profile).
...
/// A pack in week 2960, then idle weeks: the first tick of each anchors it, later ticks do not, and
/// a wallet that is down does not stop the anchor (it comes before the tick's rail calls).
#[test]
fn an_idle_week_is_anchored_by_its_first_tick_only() {
    let mut w = World::new(true);
    w.buy_pack("p");
...
/// A transition decided before a week's first tick starts the week's segment: nothing to anchor.
#[test]
fn a_week_with_a_transition_before_its_first_tick_gets_no_anchor() {
    let mut w = World::new(true);
    w.buy_pack("p");
    w.advance(WEEK_SECS);
    let week = w.week();
...
/// moves to week 2961, whose first tick anchors it.
...
/// durable anchor (so the next tick decides none), and the next tick decides a lost or torn one
/// again under the same sequence number. Either way the week ends with one applied anchor, which a
/// further restart and a restore of the snapshot taken before the week replay without adding one.
#[test]
fn a_crash_before_or_after_the_anchor_append_leaves_one_applied_anchor() {
    ...
    for (site, mode) in [
        ...
        w.refill();
        assert_eq!(w.tick().unwrap().anchored, !durable, "{mode:?}");
        w.assert_anchor_last(BASE_WEEK + 1);
...
/// The hourly snapshot of week 2962 verifies; at Monday noon of week 2963 the prune removes the
/// pack's segment (2960, due since `start(2962)`) and the first anchor's (2961, due since
/// `start(2963)`), leaving segments that hold only anchors. The issuer restarts on them, a restore
/// of the verified snapshot replays the one anchor after it, and the client's pack is still
/// re-served (MS-1). From then on the idle journal holds the segments of the current and the
/// previous week only.
#[test]
