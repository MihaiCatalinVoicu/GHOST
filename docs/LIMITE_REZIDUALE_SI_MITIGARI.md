...
| **Entitlement: tăierea jurnalului issuer-ului** (Faza 8). `FileJournal::prune` există, dar nici `ghost-issuer`, nici `ghost-issuer-ops` nu îl rulează, deci `issued.journal` crește, iar retenția de 7–14 zile (ADR-26 punctul 5) nu e aplicată | deschis (design §19.23 punctul 4; `RUNBOOK.md` §13) | o comandă de operare care taie segmentele după un instantaneu verificat mai nou |
| **Entitlement: numărătorile relay-urilor pentru R2** (Faza 8). `nullifiers.redb` ține rândurile pe săptămână, dar `ghost-relay` nu le exportă, deci verificarea independentă „suma pe sloturi ≤ token-urile de acces semnate” a reconcilierii R2 nu are încă date de la relay-uri | deschis (§19.23 punctul 4) | un export al numărătorilor săptămânale pe relay |
| **Entitlement: onion-ul issuer-ului la instalare** (Faza 8). Verificarea caută `<onion>:443` în octeții ES-ului; arată că ES-ul listează onion-ul servit de Tor, nu că îl listează ca `issuer_onion` | deschis (§19.23 punctul 4) | o comandă `ghost-issuer-ops` care afișează `issuer_onion` |
...
| **Entitlement: sesiunea de fundal fără pereche de citire** (Faza 8). O sesiune de fundal fără nicio pereche de citire se încheie înaintea primului pas al benzii de răscumpărare, deci un client ale cărui capabilități au expirat toate răscumpără doar în prim-plan | decis (Q29: sesiunea rămâne deschisă cât o nevoie de scriere așteaptă, până rulează un pas al benzii); implementarea urmează (§19.23 punctul 5) | după implementare, rândul se închide |
| **Entitlement: trialul în high-privacy mode** (Faza 8). Zilele suplimentare Geometric(1/2) ale slotului de activare pot împinge eligibilitatea token-urilor unui trial dincolo de ambele lui săptămâni, deci un invitat în high-privacy mode își poate pierde trialul | decis (Q30: zilele suplimentare se opresc la ultima săptămână a trialului); implementarea urmează (§19.23 punctul 5) | după implementare, rândul se închide |
...
| **Entitlement: lumea T2 pe sistemul complet** (Faza 8, slice-ul S10). Din T2 sunt livrate doar NI-K pe motorul Kotlin real (mutanții M3, M8, M20) și vectorii de politică; J1–J10, T2b, T2c, NI-1, NI-1d, NI-2, NI-3, S1–S4 (inclusiv S3d), mutanții M1–M21 ai lumii și poarta ei CI nu există încă. Rândurile L6 (E5, E15, E16) și sinteza din §5 care citează T2 sau S4 descriu ce va verifica acea lume | deschis (INVARIANTS T2; ADR-23, secțiunea Teste) | slice-ul S10; până la o rulare verde înregistrată a porții T2, nimic din L6 nu se afirmă în UI (CP-10) |
...
**De ce nu dispare complet:** semnătura blind scoate octeții token-ului din orice join, iar dovada de permutare și ES-ul fixat scot cheile, prețurile și perioadele alese per client (ADR-22). Rămân momentele: când plătești, când folosești prima dată un lot, când vorbește clientul cu issuer-ul. Designul le reduce la scurgerea declarată L1–L7 (INVARIANTS, nota T2); T2 verifică exact că nimic altceva nu trece și statistic cât rămâne, dar azi e livrat doar NI-K (pe motorul Kotlin real, cu vectorii de politică); J1–J10, NI-1, NI-1d, NI-2, NI-3 și verificarea statistică (S1–S3 la α = 0,001; S4 ≤ 0,25 pentru ce permite L însăși) vin cu lumea T2 pe sistemul complet (slice-ul S10, încă nelivrat; L5). Unde rândurile de mai jos citează T2 sau S1–S4, descriu ce va verifica acea lume. Nimic din această secțiune nu se afirmă în UI până la o rulare verde înregistrată a porții T2 (CP-10); textul pentru utilizator vine în Faza 13.

| ID | Reziduu | De ce rămâne | Atenuare și starea după Faza 8 |
|---|---|---|---|
| E1 | Ziua slotului de activare al primei folosiri a unui pachet (L1) | slotul estompează, nu elimină, legătura cumpărare → prima folosire pentru reluări și genesis | slot = prima graniță de zi UTC ≥ finalizare + 4 h, plus U[0, 6 h); în high-privacy mode plus zile Geometric(1/2); reînnoirile cumpărate din timp nu dau semnal; invitații: onboarding finanțat de trial și scrierea în drop la un moment tras la activare, independent de cumpărări |
...
| Operator cu issuer-ul și toate relay-urile (entitlement) | Nu poate lega o plată de un cluster de relay prin octeți, chei, prețuri sau perioade (ADR-22). Rămân momentele din scurgerea declarată L1–L7 (L6: prima folosire la un slot de activare, golurile rulărilor liniștite, momentul plății, cumpărarea după epuizare), pe care le va mărgini testul T2 și, pentru ce permite L însăși, S4 ≤ 0,25 (lumea T2 a slice-ului S10, încă nelivrată; L5) |
