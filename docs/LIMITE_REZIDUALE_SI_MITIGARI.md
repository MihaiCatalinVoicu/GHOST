...
**De ce nu dispare complet:** semnătura blind scoate octeții token-ului din orice join, iar dovada de permutare și ES-ul fixat scot cheile, prețurile și perioadele alese per client (ADR-22). Rămân momentele: când plătești, când folosești prima dată un lot, când vorbește clientul cu issuer-ul. Designul le reduce la scurgerea declarată L1–L7 (INVARIANTS, nota T2); T2 verifică exact că nimic altceva nu trece (J1–J10, NI-1, NI-1d, NI-2, NI-3, NI-K) și statistic cât rămâne (S1–S3 la α = 0,001; S4 ≤ 0,25 pentru ce permite L însăși). Nimic din această secțiune nu se afirmă în UI până la o rulare verde înregistrată a porții T2 (CP-10); textul pentru utilizator vine în Faza 13.
...
| E5 | Golurile rulărilor liniștite, cumulate per factură (L5) | un apel către issuer trebuie să aibă loc cândva; apelurile unei facturi sunt legate prin `invoice_id` | q = 1/8, extrageri independente de cumpărări; ≤ 3 biți per apel (mult mai puțin sub Doze); de obicei 2 apeluri per cumpărare, niciodată peste 6 (`RequestInvoice` cu cel mult o reîncercare, `BlindSign` cel mult 5), orice ar răspunde issuer-ul, deci ≤ 18 biți în cel mai rău caz față de un issuer care minte; biții măsurați per factură intră în raportul T2 și sunt mărginiți de S4 |
...
| E11 | Token-urile și creditele nu se recuperează din seed; drop-urile peste indexul 7 se pierd la restaurare | valoarea la purtător e locală; `next_invite_index` nu se poate recupera | după o restaurare clientul ascultă drop-urile 0..7 timp de 5 săptămâni; anunț în UI (Faza 13); recuperarea din seed ca P1 (Q13) |
...
| Operator cu issuer-ul și toate relay-urile (entitlement) | Nu poate lega o plată de un cluster de relay prin octeți, chei, prețuri sau perioade (ADR-22). Rămân momentele din scurgerea declarată L1–L7 (L6: prima folosire la un slot de activare, golurile rulărilor liniștite, momentul plății, cumpărarea după epuizare), mărginite de testul T2 și, pentru ce permite L însăși, de S4 ≤ 0,25 |
