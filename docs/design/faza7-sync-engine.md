...
- **Clock:**
  ...
  - M3, D2 and GC run only after the transport reached READY in this process, where Arti has accepted the consensus clock (σ). A forward jump therefore cannot close windows or delete tombstones early. **Superseded by §11.5 #2:** trust is checked per use (session online, and the wall clock moved with the monotonic clock since the last READY, within 1 h).
...
| Device forensics (AD-8) | The §2.3 residue; pending outbox rows; cursors and capabilities; OS `jobs.xml` (the one periodic job, no extras); OS netstats, which keep per-UID traffic volume per time bucket that follows sends and received traffic in both modes until cover traffic (residual 3 below), and batterystats, which keep job durations that grow with the work a background session drains (§11.5 #6) | request history, schedules, failures (the sync layer persists none; T20 covers what the sync layer writes outside SQLCipher, not what the OS records) |
...
Tuning T, jitter and caps from measurements; fixed-duration background jobs (so OS battery statistics stop following the work drained, LIMITE L1.1); T17 network capture with a KS test; cover stores and filler gets (P2); read bucketing / PIR-lite (P2); per-scope rotation and pooled clients with a rotation epoch; shared cursor anchors; bridges and pluggable transports.
...
> | Modifică | ADR-06 („Sync prin WorkManager”); Master Plan §6.2 (rândul Fazei 7) și §6.6 (`sync/ # WorkManager`); Spec v2.0 FR-7.3 („WorkManager and coroutines”; corectat din „FR-7.8” după raportul S8, vezi §11.5); setul de permisiuni T15. Precizează ADR-09 („loturi de dimensiune fixă”), ADR-11 și ADR-15 („interval fix + jitter”). Declară un conflict cu NFR-1 și un reziduu față de THREAT_MODEL AD-8. |
>
> ## Context
...
> 5. **Conflict NFR-1.** Cu interval de 30 s, latența DM p95 în prim-plan e de ordinul a 40 s (în fundal, ore), deci NFR-1 („DM p95 < 2 s, p99 < 5 s”; corectat din „DM p95 < 5 s” după raportul S8, vezi §11.5) **nu e îndeplinit** în Faza 7. Se măsoară și se decide în Faza 12; un interval mai scurt se aplică tuturor perechilor, altfel ritmul ar dezvălui tipul namespace-ului.
...
### 11.5 Corrections after the adversarial review S9 (override earlier sections)

Six confirmed findings of the S9 review, fixed on 2026-09-11 and verified on 2026-09-12. The five code findings each have a test that fails before the fix and passes after; #6 is a documentation correction (no test is possible). Where this subsection contradicts §0–§11.4, it wins.

1. **Own tombstones when listening starts (IN-2).** An own `done` row used to be written only by enqueue, and only in a namespace that already listened. Now every listening 0 → 1 transition (`setListening(ns, true)`, or `register(..., listen = true)` on a known write-only namespace) writes, in the same transaction, an own `done` row for every `outbox_op` of the namespace, with the enqueue statement and rule (`ceil7(today + TTL + 8)`; an existing `done` row is raised; a row in another state is left to the inbox). Later receipts raise these rows through the usual path. **Residual, declared rather than refused:** ops already deleted while the namespace was write-only left no row, so if a relay still holds such a blob, the consumer receives its own blob once after subscribing. It joins the protocol-layer boundary of §0.2: the consumer rejects it like any blob it cannot process. Refusing the transition would need a persisted per-namespace marker of recent own writes on write-only namespaces (a new AD-8 residue, against "nothing for write-only namespaces") and would block subscribing for up to TTL + 8 days. Recorded in ADR-20 point 6. Tests: `store/OwnTombstoneTest`; scenario S-L (`harness/ScenarioListenLater`, fault-free and in the exhaustive enumeration, both journal modes); mutant M14 NoOwnTombstone.
2. **Clock trust per use (§3.7).** Reaching READY once in the process is no longer enough. M3, the D1/D2/W sweep and GC trust the wall clock only while the session is online, and only while the wall clock has moved with the monotonic clock since the last successful `ensureReady`: `|Δwall − Δmono| ≤ 1 h` (`SyncEngine.clockTrusted()`, which records (wall, monotonic) at every READY). A larger step is not trusted until the next READY re-anchors the clock. A GC item that comes due while offline does nothing, and so does the final background GC. Tests: `engine/ClockTrustTest`; scenario S-K (`harness/ScenarioClockStepOffline`: READY, a transport fault, a 120-day forward step with the hourly GC due).
3. **Cursor compare-and-set (IN-3).** `commitPage` receives the cursor the request was sent with and stores the next cursor only if the stored cursor still equals it (no row = the empty cursor). Otherwise (a `remove` and `register` raced an in-flight list) the hashes are committed, the stored cursor stays, and the read lane adopts the stored cursor (`PageCommit.cursor`) and reads no further page for that event. Test: `engine/ReadLaneTest.aListInFlightAcrossRemoveAndRegisterCannotStoreItsOldCursor`.
4. **The harness checks IN-2's own-blob clause.** The oracle consumer reports "IN-2: consumer was offered its own blob" when a claim returns one of the client's own ops; mutant M14 NoOwnTombstone (the own `done` row written for another hash) is detected. The T19 mutants cited by INVARIANTS T19 and ADR-20 now exist and are detected on the T19 worlds: M15 WorkFailureFeedsReadBreaker, M16 WorkPauseSuppressesLists, M17 ContinuationsUseEventWorkers.
5. **A queued further page never holds its pair (T19, §1.4).** Only a running further page makes its pair busy. A further page still queued when its pair's next event is due is dropped and the pair's work bundle is queued, so another pair's further page, running up to its deadline, can no longer make this pair's first page late or skipped. Test: `engine/ReadLaneTest.aQueuedFurtherPageNeverHoldsItsPairPastItsNextEvent`; the T19 tests are unchanged and pass.
6. **OS statistics (AD-8).** §6.3 and LIMITE L1.1 said that netstats and batterystats show only the periodic job. In fact the OS keeps per-UID traffic volume per time bucket, which follows sends and received traffic in both modes until cover traffic (P2; §6.3 residual 3), and batterystats keeps job durations, which grow with the work a background session drains. T20's scope is what the sync layer writes outside SQLCipher (only the job entry), not everything the OS records. §6.3 and LIMITE L1.1 are corrected.

Harness bound (found while verifying these fixes, seed 18291): the seeded-world quiescence tail was capped at 400 job periods (100 h), shorter than H. An op whose last open delivery is a possible copy on a relay that never answers a check (a STALL relay), after an honest relay refused it for good, legitimately stays pending until max(first copy + H, copy + TTL − σ). The cap is now H + max(0, longest TTL − σ) + 2 days, at least 400 periods; the tail still stops at quiescence.

Also corrected (step S8 report): Appendix A cited "Spec FR-7.8" for the WorkManager requirement (it is **FR-7.3**; FR-7.8 is sensitive screens and clipboard) and gave NFR-1 as "DM p95 < 5 s" (NFR-1 is **DM p95 < 2 s and p99 < 5 s**; p95 < 5 s is NFR-2, forum post sync). ADR-20 already used the correct values; Appendix A is fixed in place.
